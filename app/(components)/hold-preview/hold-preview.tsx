                ...
                {state.payload.tags && (
                  <div className="flex flex-wrap gap-2 text-xs text-muted">
                    {state.payload.tags.map((tag) => (
                      <span key={tag} className="rounded-full bg-surface-muted px-2.5 py-1 font-mono text-[11px] uppercase tracking-wide">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex flex-wrap gap-3 text-sm font-medium">
                  {state.payload.links.map((link) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1 rounded-full border border-subtle px-3 py-1.5 transition hover:border-accent-1 hover:text-accent-1"
                    >
                      {link.label}
                    </a>
                  ))}
